...
actor ConcurrencyManager {
    ...
    func performConcurrentAnalysis(files: [String],
                                   analyzer: @escaping @Sendable (String) async -> [CodeIssue]) async -> [String: [
        CodeIssue
    ]] {
        await withTaskGroup(of: (String, [CodeIssue]).self) { group in
            for file in files {
                group.addTask {
